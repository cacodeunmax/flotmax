...
n_values_ <- seq(from=325,to=500,by=13) # plage de variation de n, le nb de noeuds
max_cap_values_ <- c(10) # plage de variation des capacités des arcs 
edge_proba_values_=c(0.1,0.15,0.2)
e_values_=seq(from=3000,to=3000,by=1)
...
def_run_simu=function(n_iter=1,e_values=e_values_,n_values=n_values_,max_cap_values=max_cap_values_,edge_prob_values=edge_proba_values_,algo="fordR",controlE=F){
  ...
  file_path <- file.path(folder_path, "simulation_results_edmondVS_Efixe.csv")
    ...
    for (max_cap in max_cap_values) {
      ...
      for (e in e_values){
        ...
        for (i in 1:n_iter){
          ...
          results_line_R <- data.frame(E = E, f = f, n = n, ExecutionTime = execution_time["elapsed"],cpp=0)
          ...
          f <- res.ford$value
          results_line_cpp <- data.frame(E = E, f = f, n = n, ExecutionTime = execution_time["elapsed"],cpp=1)
...
def_run_simu(n_iter = 5,e_values_,n_values_,max_cap_values_,edge_prob_values_,controlE=T)
